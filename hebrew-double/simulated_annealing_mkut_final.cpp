    ...
    */
    ...
    for (int loop = 0; loop < 5 && current_score > 0; loop++) {
        ...
        while (!sa.end() && current_score > 0) {
            update++;
            if (update == 0) {
                // fprintf(stderr, "世界をやり直す\n");
                world++;
                if (world >= 20) {
                    // fprintf(stderr, "この世界はもうだめ\n");
                    break;
                }
    ...
    fprintf(stderr, "INITIAL ROOOM %d\n", vertex[0][0]);
    int ok = 0;
    int ng = 0;
    for (int p = 0; p < plans; p++) {
        for (int i = 0; i <= plan_length; i++) {
            if (vertex[p][i] % 4 == result[p][i]) {
                ok++;
            } else {
                ng++;
            }
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            if (vvi[i][j] == vvi[j][i]) ok2++;
            else ng2++;
        }
    }
    fprintf(stderr, "ok2 = %d, ng2 = %d\n", ok2, ng2);
